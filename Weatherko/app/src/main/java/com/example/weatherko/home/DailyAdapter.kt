...
import com.example.weatherko.databinding.ItemRvDaliyBinding
import com.example.weatherko.model.Daily
...
class DailyAdapter (
    ...
    override fun onBindViewHolder(holder: DailyAdapter.ViewHolder, position: Int) {
        ...
        Glide.with(context).load(AppCompatResources.getDrawable(context,Helper.image.get(daily[position].weather[0].icon)!!))
            .error(AppCompatResources.getDrawable(context,com.example.weatherko.R.drawable.twotone_error_24)).into(holder.binding.ivDaily)
        Log.i(TAG, "onCreateView: $daily[position].weather[0].icon)")
