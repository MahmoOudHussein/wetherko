...
import com.example.weatherko.databinding.ItemRvHourlyBinding
import com.example.weatherko.model.Hourly
...
class HourlyAdapter(private var hourly: List<Hourly> = listOf<Hourly>(),
    ...
    override fun onBindViewHolder(holder: HourlyAdapter.ViewHolder, position: Int) {
        ...
        Glide.with(context).load(AppCompatResources.getDrawable(context,Helper.image.get(hourly[position].weather[0].icon)!!))
            .error(AppCompatResources.getDrawable(context,com.example.weatherko.R.drawable.twotone_error_24)).into(holder.binding.ivHourly)
